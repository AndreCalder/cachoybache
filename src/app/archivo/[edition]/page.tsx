                ...
                (
                    <>
                                ...
                                <div className={`fixed top-0 bottom-0 left-0 right-0 bg-black/50 z-50 flex justify-center items-center ${!lightBoxActive ? 'hidden' : ''}`}>
                                    ...
                                    <div className="flex z-50 items-center gap-5">
                                        ...
                                        <div className="w-full md:w-[603px] h-fit min-h-64 bg-black p-5 grid grid-cols-12 relative">
                                            <XIcon className="w-6 h-6 cursor-pointer text-white absolute top-5 right-5" onClick={() => setLightBoxActive(false)} />
                                            <div className="col-span-12 md:col-span-7">
                                                <Image src={editionData.gallery[activeImage].url} alt="activeImage" width={300} height={300} />
                                            </div>
                                            <div className="col-span-12 md:col-span-5 flex flex-col justify-end">
                                                <h1 className='text-sm text-white font-light italic'>{editionData.gallery[activeImage].title}</h1>
                                                <p className='text-xs text-white'>/ Fotografía por {editionData.gallery[activeImage].author}</p>
                                                <p className='text-xs text-white'>// Sección "{editionData.gallery[activeImage
                                                ].section}"</p>
                                            </div>
                                        </div>
                        ...
                        </div>
                        {
                            portada != null && (
                                <div className="w-full grid grid-cols-12 md:px-12 pb-6 gap-2">
                                    <div className="col-span-12 lg:col-span-4 min-h-60 flex items-center justify-center py-3" style={{ backgroundColor: editionData.covers[portada].bgColor }}>
                                        <Image className='h-fit' src={editionData.covers[portada].url} alt={portada} width={200} height={300} />
                                    </div>
                                    {
                                        editionData.gallery.length > 0 && (
                                            <div className="col-span-12 lg:col-span-8 flex flex-wrap justify-between gap-2" >
                                                {
                                                    editionData.gallery.slice(0, 11).map((image: { url: any; }, index: number) => {
                                                        return (
                                                            <img className={`h-[126px] cursor-pointer transition-all object-contain hover:scale-105 aspect-auto`} src={image.url || ''} alt={`image-${index}`} onClick={() => handleImageClick(index)} key={index} />
                                                        )
                                                    })
                                                }
                                            </div>
                                            ...
                                            <div className="col-span-12 flex flex-wrap justify-between gap-2">
                                                {
                                                    remainingImages.map((image, index) => {
                                                        return (
                                                            <img className={`h-[126px] cursor-pointer transition-all object-contain hover:scale-105 aspect-auto`} src={image.src || ''} alt={`image-${index}`} onClick={() => handleImageClick(index)} key={index} />
                                                        )
                                                    })
                                                }
                                            </div>
                        ...
                        <div className="w-full flex flex-col px-5 py-3 gap-y-1 md:px-12 min-h-40">
                            ...
                            <p className='italic'>Portada rosa/ Fotografía por Isabel Barba - Portada azul/ Fotografía por María Correa - Portada blanco/ Fotografía por Hugo Moreno - Portada verde/ Fotografía por Mariana Guerrero</p>
                        </div>
                    </>
                )
            }
