...
function Archivo() {
  ...
  const getData = async () => {
    ...
    setActiveImages(
      res.data.map((edition: { covers: any[] }) => edition.covers[0])
    );
      ...
      <div className="min-h-lvh w-full grid grid-cols-12 px-5 md:px-12 pt-6">
        {archive.map((edition, index) => {
          return (
            <div
              key={index}
              className="col-span-12 md:col-span-6 lg:col-span-3 flex flex-col gap-1 p-4"
            >
              <div className="w-full flex justify-center">
                <Image
                  alt={`${edition.title}-${index}`}
                  className="border-[1px] border-solid border-black"
                  src={activeImages[index].url}
                  width={450}
                  height={600}
                />
              </div>
              <p className="font-light italic">{edition.subtitle}</p>
              <h2 className="font-normal text-2xl tracking-wider underline">
                {edition.title}
              </h2>
                ...
                {edition.covers.map((variant: any, variantIndex: number) => {
                  return (
                    <div
                      ...
                      key={variantIndex}
                      className="h-4 w-4 cursor-pointer border-[1px] border-solid border-black rounded-full"
              ...
              <div className="w-full grid grid-cols-12 gap-4">
                <Link
                  href={`/archivo/${edition._id.$oid}?portada=${activeIndexes[index]}`}
                  className="col-span-6 text-center w-full border-[2px] ocrb border-solid border-black px-4 py-2"
                >
                  Ver más
                </Link>
              </div>
