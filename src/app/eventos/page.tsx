...
function Eventos() {
  ...
  const getData = async () => {
    const eventsRes = await getEvents(checkedIndex == -1 ? undefined : creativxs[checkedIndex]._id.$oid)
    ...
    <div className='w-full pt-24 h-svh flex flex-col'>
      <div className={`w-full min-h-24 px-5 md:px-12 flex items-center justify-start py-8 border-b-4 border-black`}>
        <div className="flex flex-col gap-2">
          ...
          <p className='tracking-[0.3em] font-bold sm:text-3xl acumin italic'>CALENDARIO DE EVENTOS</p>
        </div>
      </div>
      <div className="w-full flex-1 grid grid-cols-12">
        <div className="col-span-12 md:col-span-4 border-b-[2px] md:border-b-0 border-r-0 md:border-r-2 border-black  py-5 px-5 md:pl-12">
          ...
          <p className='tracking-[0.3em] font-bold sm:text-3xl acumin italic'>LXS CREATIVXS</p>

          <p className='font-lighter acumin italic'>Filtra por evento / creativx</p>
          <div className="flex items-center space-x-2 py-2">
            <Checkbox className='border-black data-[state=checked]:bg-black rounded-none ' checked={checkedIndex == -1} onCheckedChange={(e) => { check(-1); }} />
            <label
              htmlFor="terms"
          ...
          </div>
          {
            creativxs.map((creativx, index) => (
              <div key={index} className="flex items-center space-x-2 py-2">
                <Checkbox className='border-black data-[state=checked]:bg-black rounded-none' checked={checkedIndex == index} onCheckedChange={(e) => { check(index); }} />
                <label
                  htmlFor="terms"
                  className="text-sm font-extralight leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70 ocrb"
                >
